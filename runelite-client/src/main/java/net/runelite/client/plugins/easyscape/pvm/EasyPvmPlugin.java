...
@PluginDescriptor(
        name="<html><font color=\"Red\">EasyPVM</font></html>",
        description = "EasyPvM.",
        tags = {"EasyPVM", "easy"}
)
...
public class EasyPvmPlugin extends Plugin {
    ...
    public void onMenuEntryAdded(MenuEntryAdded event) {
        ...
        if (config.getShiftWalk() && shiftModifier && option.contains("attack"))
        {
        ...
        if(config.getRemoveFreezePlayerCoX() &&  client.getVar(Varbits.IN_RAID) == 1) {
            for (int i = entries.length - 1; i >= 0; i--) {
                    for (String spell : FreezeSpells) {
                        System.out.println("Target: "+Text.removeTags(entries[i].getTarget().toLowerCase()));
                        System.out.println("Type: "+entries[i].getType());
        ...
        if(config.getRemoveFreezePlayerToB() &&  client.getVar(Varbits.THEATRE_OF_BLOOD) == 1) {
            for (int i = entries.length - 1; i >= 0; i--) {
                for (String spell : FreezeSpells) {
                    System.out.println("Target: " + Text.removeTags(entries[i].getTarget().toLowerCase()));
                    System.out.println("Type: " + entries[i].getType());
